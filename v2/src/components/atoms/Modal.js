...
const Modal = ({ gno, editGrapeIdx }) => {
	const { modalTitle, modalContent, isContentChangeSuccess } = useSelector(
		({ grape, loading }) => ({
	...
	const editGrapeContent = () => {
		dispatch(
			changeGrapeContent({ gno, idx: editGrapeIdx, title, content })
		);
	};
	...
	return (
		<ModalBackground>
			<GrapeModal>
				...
				<SubmitBtn onClick={editGrapeContent}>입력</SubmitBtn>
			</GrapeModal>
		</ModalBackground>
	);
};
